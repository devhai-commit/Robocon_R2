...
| AI | YOLO (yolo26n_best — .pt / .onnx / .engine TensorRT) |
...
BT Root (Sequence)
├── Phase 0: Standby
│   ├── MoveArmBehavior("Box_Arm_Home")       ← Gập tay về home
│   └── WaitForStartSignalBehavior            ← Chờ GUI (priority_col, target_boxes_list)
├── Phase 1: Init
│   └── SetBlackboardVariable("Set_Target_2_1")
└── Phase 2: Grid Mission Loop
    └── KeepRunningUntilSuccess
         └── Mode_Selector
              ├── EXIT Goal (real_box≥2 AND at_exit_cell) → SUCCESS
              ├── FIND_Exit (real_box≥2) → DecideNext(EXIT) → move
...
- `FIELD_CONFIGS` dict: cấu hình sân `red`/`blue` — bản đồ độ cao 3×4 ô, hệ số `lat` (dấu dy), `post_ramp_yaw`, `tool_arm_side`
- `CLASS_LABELS` / `CLASS_PICK_INTENT`: mapping class ID YOLO → tên → hành động (pick/avoid/skip)
- `PREDEFINED_POSES` tay máy (cũng có trong esp32_arm_action_server)
...
| Class | Mục đích |
|-------|----------|
| `WaitForStartSignalBehavior` | Parse JSON từ `gui_start_raw` → ghi `priority_col`, `target_boxes_list` |
| `DecideNextGridCellAction` | Thuật toán chọn ô tiếp theo: ưu tiên `priority_col`, chưa visit, dùng manhattan distance |
...
| `KeepRunningUntilSuccess` | Decorator: luôn trả RUNNING cho đến khi child trả SUCCESS |
...
| `build_move_subtree(ros_node, ...)` | Sequence: Turn → WallAlign → FollowTarget → GraspLogic → ElevationCheck → Climb → Move |
| `build_pick_and_place_sequence(...)` | Sequence gắp hộp dựa trên preset cao độ hiện tại |
...
WaitForStartSignalBehavior
    │ parse JSON → blackboard["priority_col"], ["target_boxes_list"]
    ▼
DecideNextGridCellAction
    │ chọn ô (col, row) chưa visited, ưu tiên priority_col
    ▼
TurnToTargetCellBehavior ──► odom_nav_server ──► /cmd_vel → RockerBogieController
    │ (navigate_to_pose: xoay tại chỗ về hướng ô đích)
    ▼
WallAlignmentBehavior ──────► wall_alignment_server
    │ Phase1: /scan LiDAR → xoay vuông góc
    │ Phase2: snap yaw → /set_pose → EKF reset yaw
    │ Phase3: tiến vào khoảng cách chuẩn
    ▼
FollowTargetBehavior ───────► tracking_server
    │ YOLO (TRT) nhận diện hộp trong frame RealSense
    │ Kalman filter → strafe sang ngang (cmd_vel.y)
    │ Depth sensor → tiến vào 300mm (cmd_vel.x)
    │ Trả về: "real" / "fake" / "empty"
    ▼
Box Decision (Selector):
  ├─ [real]  DeltaPickSequenceBehavior
  │              ├─ MoveArmBehavior → arm_action_server → /arm_controller/commands
  │              │  (pre_grasp → extend → close → lift → retract)
  │              └─ IncrementRealCountAction (real_box_count++)
  ├─ [fake]  MarkAsObstacleAction
  └─ [empty] MarkAsVisitedAction
    ▼
HasElevationChangeCondition → tra bản đồ FIELD_CONFIGS
    │ Nếu bậc thay đổi:
    ▼
DynamicClimbStepBehavior ───► step_climb_server
    │ Tiến thẳng v = ±0.3 m/s
    │ Đợi pitch > 5° rồi < 1.5°
    ▼
MoveRelativeOdomBehavior ───► odom_nav_server (di chuyển vào ô 1.2m / 0.3m nếu leo)
    │
    ▼ (vòng lặp: DecideNextGridCell → ...)
    
  Khi real_box_count ≥ 2 → chuyển chế độ FIND_EXIT → về (1,4) hoặc (3,4)
```
